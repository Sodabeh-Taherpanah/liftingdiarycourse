...
export function WorkoutCard({ id, title, startedAt, completedAt, workoutExercises }: WorkoutCardProps) {
  const duration = completedAt ? differenceInMinutes(completedAt, startedAt) : null

  return (
    <Link href={`/dashboard/workout/${id}`} className='block hover:opacity-80 transition-opacity'>
      <Card>
